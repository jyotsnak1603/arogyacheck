"""
Views for the patient assessment and reporting workflow.
Handles questionnaire submission, risk calculation trigger, and visualization rendering.
"""
...
def questionnaire_view(request):
    """
    Handles the health assessment form submission.
    Creates or updates the HealthProfile and processes the health Questionnaire.
    Triggers the risk analysis engine and creates a RiskReport.
    """
...
def report_view(request):
    """
    Displays the latest health risk report to the patient.
    Renders an interactive health trend chart using Plotly based on historical scores.
    """
    try:
        health_profile = HealthProfile.objects.get(patient=request.user)
